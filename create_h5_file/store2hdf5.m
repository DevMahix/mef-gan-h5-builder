function [ curr_dat_sz ] = store2hdf5( filename,data,create,startloc,chunksz )
% *data* is W*H*C*N matrix of images should be normalized (e.g. to lie between 0 and 1) beforehand
% *create* [0/1] specifies whether to create file newly or to append to previously created file
%          useful to store information in batches when a dataset is too big to be held in memory  (default: 1)
...
% chunksz (used only in create mode), specifies number of samples to be stored per chunk 
% (see HDF5 documentation on chunking) for creating HDF5 files with unbounded maximum size - TLDR; higher chunk sizes allow faster read-write operations
